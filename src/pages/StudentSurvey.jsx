...
function StudentSurvey() {
    ...
    const studentSurveyEnabled = useQuery(api.studentSurvey.getSetting,
        schoolId ? { key: "studentSurveyEnabled", schoolId } : { key: "studentSurveyEnabled" }
    );
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (isStartMode) {
                finalId = await createStudentMutation({
    ...
    if (studentSurveyEnabled === false) {
        return (
            <div className="survey-closed-container">
                ...
                <h2>Survey Closed</h2>
                <p>The student survey is currently not accepting responses.</p>
    ...
    // Check for schoolId
    if (!schoolId && !loading && !routeStudentId) {
        return (
